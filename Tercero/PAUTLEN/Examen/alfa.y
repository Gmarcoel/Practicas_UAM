...
%type <atributos> bucle_do_inicio
%type <atributos> bucle_do_sentencia
...
bucle: bucle_exp TOK_PARENTESISDERECHO TOK_LLAVEIZQUIERDA sentencias TOK_LLAVEDERECHA 
    ...
    |
    bucle_do_sentencia TOK_LLAVEDERECHA TOK_WHILE TOK_PARENTESISIZQUIERDO exp TOK_PARENTESISDERECHO TOK_PUNTOYCOMA
    {
        if ($5.tipo == INT) {
        ...
        while_exp_pila(out, $5.es_direccion, $1.etiqueta);
        while_fin(out, $1.etiqueta);
...
/************************************/
/************************************/
/*   <bucle>::= do{ <sentencias> } while (<exp>);    */
bucle_do_inicio: TOK_DO
    {
        etiq_actual = etiqueta++;
        $$.etiqueta = etiq_actual;
        while_inicio(out, etiq_actual);
...
bucle_do_sentencia: bucle_do_inicio TOK_LLAVEIZQUIERDA sentencias 
    {
        $$.etiqueta = $1.etiqueta;
    };
