...
Status tree_node_insert(BSTree * tree, BTNode * node, const void * elem);
...
{
    if(!node) return 0;
    if(tree_node_depth(node->left)>tree_node_depth(node->right)) return tree_node_depth(node->left)+1;
...
Status tree_insert(BSTree *tree, const void *elem)
{
    ...
    return tree_node_insert(tree,tree->root,elem);
}
Status tree_node_insert(BSTree * tree, BTNode * node, const void * elem)
{
    if(!tree||!elem) return ERROR;
    ...
    {
        node = bt_node_new();
        node->info = tree->copy_element(elem);
        return OK;
    }
    if(tree->cmp_element(node->info,elem)<=0) tree_node_insert(tree,node->left,elem);
    else tree_node_insert(tree,node->right,elem);
    return OK;
}
/*** RECORRIDOS ***/
