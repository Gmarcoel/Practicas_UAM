...
void screen_paint(){
  ...
  if (__data){
    ...
    for (src=__data; src < (__data + TOTAL_DATA - 1); src+=COLUMNS){
      ...
      for (i=0; i<COLUMNS; i++){
	if (dest[i] == BG_CHAR){
	  printf("\033[0;31;41m%c\033[0m", dest[i]); /* fg:blue(34);bg:blue(44) printf("\033[0;34;44m%c\033[0m", */
	}else{
	  printf("\033[0;30;47m%c\033[0m", dest[i]); /* fg:black(30);bg:white(47) printf("\033[0;30;47m%c\033[0m"*/
	}
      }
      printf("\n");
    }
